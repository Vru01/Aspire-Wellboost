...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    const cleanUserId = u.id || u._id;
    api.cart.getCart(cleanUserId)
      .then((data) => {
  ...
  const handleRazorpayPayment = async () => {
    ...
    try {
      ...
      const orderData = await api.orders.createRazorpayOrder(formattedCartItems);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-gradient-to-r from-cyan-500 to-green-600 pt-24 pb-10 px-6">
        <div className="max-w-5xl mx-auto">
          <button onClick={() => navigate('/')} className="flex items-center gap-2 text-white/60 hover:text-white transition-colors text-sm mb-4">
